def main():
    ...
    try:
        ...
        flour_sacks = sugar_sacks = yeast_blocks = 0

    delivery_choice = input("Require Freight Delivery ($150 flat)? (Y/N): ").strip().upper()
    wants_deliv = delivery_choice == 'Y'

    # Calculations
    f_tot = flour_sacks * FLOUR_PRC
    s_tot = sugar_sacks * SUGAR_PRC
    y_tot = yeast_blocks * YEAST_PRC
    freight_fee = FREIGHT_RATE if wants_deliv else 0.0
