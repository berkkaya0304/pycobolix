"""
Fresh Mart Supermarket Checkout
Converted from COBOL (supermarket_checkout.cbl) to Python
"""
...
def main():
    ...
    for idx in range(1, 4):
        item_name = input(f"Item {idx} Name: ")
    ...
    for name, qty, sub in items:
        print(f"{name} x {qty}    ${sub:>9,.2f}")
    print("---------------------------------------")
    print(f"Subtotal:        ${gross_total:>9,.2f}")
    print(f"Tax (2%):        ${tax_amount:>9,.2f}")
    print("---------------------------------------")
    print(f"TOTAL BALANCE:   ${net_total:>9,.2f}")
    print("=======================================")
