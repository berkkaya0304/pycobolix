"""
Metro Lock & Key - Locksmith Service
Converted from COBOL (locksmith_service.cbl) to Python
"""
...
def main():
    ...
    print("----------------------------------------")
    print(f"Service Call Trip:  ${trip_fee:>9,.2f}")
    print(f"Unlock Labor Rate:  ${labor_fee:>9,.2f}")
    if keys_cut > 0:
        print(f"Keys Cut ({keys_cut}):       ${keys_fee:>9,.2f}")
    if after_hours == "Y":
        print(f"After-Hours Fee:    ${night_surchg:>9,.2f}")
    print("----------------------------------------")
    print(f"TOTAL BALANCE:      ${total_chg:>9,.2f}")
    print("========================================")
