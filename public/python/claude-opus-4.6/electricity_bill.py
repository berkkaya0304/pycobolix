"""
Grid-Connect Energy Billing - Electricity Bill
Converted from COBOL (electricity_bill.cbl) to Python
"""
...
def main():
    ...
    print("-----------------------------------------")
    print(f"Peak Usage ({peak_kwh} kWh):     ${peak_cost:>12,.2f}")
    print(f"Off-Peak Usage ({off_peak_kwh} kWh): ${off_peak_cost:>12,.2f}")
    print("-----------------------------------------")
    print(f"Gross Energy Charge:      ${sub_total:>12,.2f}")
    print(f"Solar Credit ({solar_credit} kWh): -${solar_deduct:>11,.2f}")
    print("-----------------------------------------")
    if total_due < 0:
        print(f"CREDIT APPLIED TO ACCT:   ${total_due:>12,.2f}")
    else:
        print(f"TOTAL CURRENT CHARGES:    ${total_due:>12,.2f}")
    print("=========================================")
