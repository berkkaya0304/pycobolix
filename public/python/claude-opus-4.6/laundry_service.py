"""
Fresh & Clean Dry Cleaners
Converted from COBOL (laundry_service.cbl) to Python
"""
...
def main():
    ...
    print("-----------------------------------------")
    if shirts_qty > 0:
        print(f"{shirts_qty} x Shirts:       ${shirt_cost:>9,.2f}")
    if pants_qty > 0:
        print(f"{pants_qty} x Pants:        ${pant_cost:>9,.2f}")
    if suits_qty > 0:
        print(f"{suits_qty} x Suits:        ${suit_cost:>9,.2f}")
    print("-----------------------------------------")
    if rush_fee > 0:
        print(f"Rush Service Fee: ${rush_fee:>9,.2f}")
    print(f"GRAND TOTAL:      ${total_bill:>9,.2f}")
    print("-----------------------------------------")
