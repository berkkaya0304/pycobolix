"""
Aero Capture Drone Rentals
Converted from COBOL (drone_rental.cbl) to Python
"""
...
def main():
    ...
    insure_wvt = input("Add Crash Insurance ($25/day)? (Y/N): ").strip().upper()
    ...
    print("----------------------------------------")
    print(f"Drone Flight Time:  ${drone_tot:>9,.2f}")
    if extra_batt > 0:
        print(f"Extra Power Packs:  ${batt_tot:>9,.2f}")
    if insure_wvt == "Y":
        print(f"Crash Insurance:    ${insure_tot:>9,.2f}")
    print("----------------------------------------")
