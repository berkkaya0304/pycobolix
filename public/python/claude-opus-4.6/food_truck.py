...
Converted from COBOL (food_truck.cbl) to Python
"""
...
def main():
    ...
    print("----------------------------------------")
    if taco_qty > 0:
        print(f"{taco_qty}x Tacos:            ${taco_cost:>9,.2f}")
    if burrito_qty > 0:
        print(f"{burrito_qty}x Burritos:         ${burrito_cost:>9,.2f}")
    if drink_qty > 0:
        print(f"{drink_qty}x Drinks:           ${drink_cost:>9,.2f}")
    if add_guac == "Y":
        print(f"   Side of Guac:        ${guac_cost:>9,.2f}")
    print("----------------------------------------")
    print(f"TOTAL ORDER:            ${total_cost:>9,.2f}")
    print("========================================")
