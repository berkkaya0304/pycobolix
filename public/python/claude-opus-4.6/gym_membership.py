"""
Iron Fit Studio - Gym Membership Registration
Converted from COBOL (gym_membership.cbl) to Python
"""
...
def main():
    ...
    while run_loop.upper() == "Y":
        ...
        print(f"Initiation Fee:  ${init_fee:>9,.2f}")
        if discounts > 0:
            print(f"Promo Discount:  -${discounts:>8,.2f}")
        print(f"Plan Fee:        ${base_plan:>9,.2f}")
        if pt_fee > 0:
            print(f"Trainer Fee:     ${pt_fee:>9,.2f}")
        print("--------------------------------------")
        print(f"DUE TODAY:       ${total_pay:>9,.2f}")
        print("======================================")

        run_loop = input("Register another member? (Y/N): ").strip().upper()
