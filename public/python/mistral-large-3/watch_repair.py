def watch_repair():
    ...
    watch_brand = input("Watch Brand: ").strip()
    luxury_tier = input("Is this a luxury timepiece (+25%)? (Y/N): ").strip().upper() == 'Y'
    fix_battery = input("Replace Battery ($20)? (Y/N): ").strip().upper() == 'Y'
    fix_strap = input("Replace/Fix Strap ($35)? (Y/N): ").strip().upper() == 'Y'
    fix_polish = input("Surface Polish ($45)? (Y/N): ").strip().upper() == 'Y'

    batt_fee = 20.00 if fix_battery else 0.00
    strap_fee = 35.00 if fix_strap else 0.00
    polish_fee = 45.00 if fix_polish else 0.00

    sub_tot = batt_fee + strap_fee + polish_fee
    luxury_surchg = sub_tot * 0.25 if luxury_tier else 0.00
