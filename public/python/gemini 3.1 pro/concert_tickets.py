def main():
    ...
    try:
        ...
        vip_qty = 0
    wants_parking = input("Add VIP Parking Pass ($40)? (Y/N): ").strip().upper() == 'Y'
    ...
    if gen_adm_qty > 0:
        print(f"{gen_adm_qty}x General Adm:   ${ga_total:8.2f}")
    if vip_qty > 0:
        print(f"{vip_qty}x VIP Pass:      ${vip_total:8.2f}")
    if wants_parking:
        print(f"1x Parking Pass: ${parking_fee:8.2f}")
        
    print("--------------------------------------")
    print(f"Service Fees(8%):${processing_fee:8.2f}")
    print(f"TOTAL DUE:       ${grand_sum:8.2f}")
    print("**************************************")
