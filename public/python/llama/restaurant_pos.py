# RESTAURANT-POS - Starlight Restaurant POS
...
def main():
    ...
    while more_items.upper() == 'Y' and i < 5:
        ...
        i += 1
        if i < 5:
            more_items = input("Add another item? (Y/N): ").strip()
        else:
    ...
    for name, qty, price, subtot in items:
        if qty > 0:
            print(f"{name:<15}   {qty}   ${subtot:,.2f}")
    print("-------------------------------------------")
    print(f"SUBTOTAL:         ${food_tot:,.2f}")
    print(f"TAX:              ${tax_amt:,.2f}")
    print(f"GRATUITY:         ${tip_amt:,.2f}")
    print("===========================================")
