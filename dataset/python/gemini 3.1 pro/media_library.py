def main():
    ...
    item_type = input("Item Type (1=Book, 2=DVD, 3=Laptop): ").strip()
    try:
    ...
    print(f"Days Late: {days_overdue:03d}")
    print("----------------------------------------")
    
    if days_overdue == 0:
        print("Item returned on time. No fines.")
    else:
        print(f"Late Fine:         ${calc_fine:6.2f}")
        if process_fee > 0:
            print(f"Processing Fee:    ${process_fee:6.2f}")
        print("----------------------------------------")
        print(f"TOTAL DUE:         ${total_bill:6.2f}")
