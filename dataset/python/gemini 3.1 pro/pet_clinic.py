def main():
    print("=== PAWS & CLAWS VET CLINIC ===")
    pet_name = input("Pet Name: ")
    pet_type = input("Pet Type (Dog/Cat/Bird/etc): ")
    owner_name = input("Owner Name: ")

    try:
        vaccine_qty = int(input("Number of Vaccines given ($25 each): "))
    except ValueError:
        vaccine_qty = 0
        
    blood_test = input("Perform Blood Work? ($100) (Y/N): ").strip().upper() == 'Y'
    
    try:
        medication_amt = float(input("Cost of Dispensed Medications ($): "))
    except ValueError:
        medication_amt = 0.0

    consult_fee = 65.00
    vaccine_fee = vaccine_qty * 25.00
    test_fee = 100.00 if blood_test else 0.0

    sub_total = consult_fee + vaccine_fee + test_fee + medication_amt
    tax = sub_total * 0.05
    grand_total = sub_total + tax

    print("\n=========================================")
    print("           CLINIC INVOICE                ")
    print("=========================================")
    print(f"Patient: {pet_name} ({pet_type})")
    print(f"Owner:   {owner_name}")
    print("-----------------------------------------")
    print(f"General Consultation: ${consult_fee:9.2f}")
    
    if vaccine_fee > 0:
        print(f"Vaccinations ({vaccine_qty}):    ${vaccine_fee:9.2f}")
        
    if test_fee > 0:
        print(f"Laboratory/Blood:     ${test_fee:9.2f}")
        
    if medication_amt > 0:
        print(f"Prescription Meds:    ${medication_amt:9.2f}")
        
    print("-----------------------------------------")
    print(f"SubTotal:             ${sub_total:9.2f}")
    print(f"Tax (5%):             ${tax:9.2f}")
    print("=========================================")
    print(f"TOTAL BALANCE DUE:    ${grand_total:9.2f}")
    print("=========================================")

if __name__ == "__main__":
    main()
