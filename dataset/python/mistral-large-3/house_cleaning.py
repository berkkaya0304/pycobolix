def house_cleaning_quote():
    print("--- SPARKLE MAIDS QUOTE ---")
    sq_footage = int(input("Approximate Sq Footage of house: "))
    has_pets = input("Do you have pets? (Y/N): ").upper() == 'Y'
    deep_clean = input("Require initial deep clean? (Y/N): ").upper() == 'Y'

    base_rate = sq_footage * 0.10
    pet_surcharge = 30.00 if has_pets else 0.00
    dc_surcharge = base_rate * 0.50 if deep_clean else 0.00
