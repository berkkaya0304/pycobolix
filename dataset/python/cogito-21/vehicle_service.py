def main_garage():
    ...
    oil_change = input("Oil Change ($45)? (Y/N): ").upper()
    tire_rotate = input("Tire Rotation ($25)? (Y/N): ").upper()
    inspection = input("State Inspection ($35)? (Y/N): ").upper()
    labor_hours = float(input("Estimated Labor Hours ($80/hr): "))

    labor_cost, parts_cost, total_estimate = calc_estimate(oil_change, tire_rotate, inspection, labor_hours)
    print_invoice(license_plate, mileage, oil_change, tire_rotate, inspection, labor_cost, parts_cost, total_estimate)

def calc_estimate(oil_change, tire_rotate, inspection, labor_hours):
...
def print_invoice(license_plate, mileage, oil_change, tire_rotate, inspection, labor_cost, parts_cost, total_estimate):
    print("\n========================================")
