"""
Airline Reservation - Flight Booking
Converted from COBOL (flight_booking.cbl) to Python
"""
...
def main():
    ...
    while continue_prog.upper() == "Y":
        ...
        print(f"Base Fare:       ${base_fare:>9,.2f}")
        if class_surcharge > 0:
            print(f"Class Upgrade:   ${class_surcharge:>9,.2f}")
        if luggage_fee > 0:
            print(f"Baggage Fees:    ${luggage_fee:>9,.2f}")
        print("---------------------------------------------")
        print(f"Taxes (10%):     ${tax_amount:>9,.2f}")
        print(f"TOTAL FARE:      ${grand_total:>9,.2f}")
        print("=============================================")

        continue_prog = input("Book another flight? (Y/N): ").strip().upper()
