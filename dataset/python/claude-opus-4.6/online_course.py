"""
E-Learning Registration - Online Course
Converted from COBOL (online_course.cbl) to Python
"""
...
def main():
    ...
    cert_fee = 50.00 if wants_cert == "Y" else 0.0
    tutor_fee = 150.00 if wants_tutor == "Y" else 0.0
    discount_amt = base_fee * 0.10 if is_alumni == "Y" else 0.0
    ...
    print("--------------------------------------")
    print(f"Tuition Fee:      ${base_fee:>12,.2f}")
    if cert_fee > 0:
        print(f"Certificate Fee:  ${cert_fee:>12,.2f}")
    if tutor_fee > 0:
        print(f"Tutoring Add-on:  ${tutor_fee:>12,.2f}")
    if discount_amt > 0:
        print(f"Alumni Discount: -${discount_amt:>11,.2f}")
    print("--------------------------------------")
    print(f"TOTAL ENROLLMENT: ${final_due:>12,.2f}")
    print("======================================")
