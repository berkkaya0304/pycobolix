"""
Snow Peak Resort - Ski Resort
Converted from COBOL (ski_resort.cbl) to Python
"""
...
def main():
    ...
    gear_fee = 45.00 if rent_gear == "Y" else 0.0
    helmet_fee = 15.00 if rent_helmet == "Y" else 0.0
    ...
    print("----------------------------------------")
    print(f"Lift Ticket Access:  ${lift_ticket:>9,.2f}")
    if gear_fee > 0:
        print(f"Equipment Rental:    ${gear_fee:>9,.2f}")
    if helmet_fee > 0:
        print(f"Helmet Rental:       ${helmet_fee:>9,.2f}")
    print("----------------------------------------")
    print(f"TOTAL PAID:          ${total_cost:>9,.2f}")
