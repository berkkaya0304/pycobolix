def main():
    ...
    wants_oil = input("Synthetic Oil Change ($65)? (Y/N): ").strip().upper() == 'Y'
    wants_tire = input("Tire Rotation & Balance ($40)? (Y/N): ").strip().upper() == 'Y'
    
    brake_choice = input("Brakes (1=Front $150, 2=Rear $150, 3=All $280, 0=None): ").strip()
    
    oil_fee = 65.00 if wants_oil else 0.00
    tire_fee = 40.00 if wants_tire else 0.00
    
    brake_fee = 0.00
    if brake_choice == '1' or brake_choice == '2':
        brake_fee = 150.00
    elif brake_choice == '3':
        brake_fee = 280.00
